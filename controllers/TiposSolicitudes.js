...
module.exports = {
  ...
  createTiposSolicitudes: (req, res) => {
    ...
    if (
      ...
      db.query(
        `CALL sp_AgregarTiposCreditos('${IdUsuario}', '${Descripcion}','${OCP}','${OLP}' )`,
        (err, result) => {
          if (err) {
            console.log(err)
  ...
  getTiposSolicitudes: (req, res) => {
    db.query(`CALL sp_ListadoTiposCreditos()`, (err, result) => {
      if (err) {
        return res.status(500).send({
          error: "Error",
        });
...
//   },
  ...
  modifyTiposSolicitudes: (req, res) => {
    ...
    if (IdUsuarioModificador == null || /^[\s]*$/.test(IdUsuarioModificador)) {
      ...
      db.query(
        `CALL sp_ModificaTiposCredito('${IdDescripcion}','${Descripcion}','${IdUsuarioModificador}','${OCP}','${OLP}')`,
        (err, result) => {
          if (err) {
            return res.status(500).send({
              error: "Error",
  ...
  deleteTiposSolicitudes: (req, res) => {
    ...
    db.query(
      `CALL sp_BajaLogicaTiposCredito('${IdDescripcion}', '${IdUsuarioModificador}')`,
      (err, result) => {
        if (err) {
          return res.status(500).send({
            error: "Error",
